...
HRESULT Application::Initialise(HINSTANCE hInstance, int nCmdShow)
{
	...
	CreateDDSTextureFromFile(_pd3dDevice, L"Resources\\brick.dds", nullptr, &_pTowerTex);
...
void Application::InitObjects()
{
	...
	Geometry towerGeometry = OBJLoader::Load("Objects/building.obj", _pd3dDevice);

	appearance = new Appearance(towerGeometry, shinyMaterial);
	appearance->SetTextureRV(_pTowerTex);

	transform = new Transform();
	transform->SetPosition(0.0f, -2.0f, 0.0f);
	transform->SetScale(1.0f, 2.5f, 1.0f);
	//transform->SetRotation(XMConvertToRadians(0.0f), 0.0f, 0.0f);
	
	particleModel = new PlaneParticleModel(transform, 1.0f);
	XMFLOAT3 pos = transform->GetPosition();
	particleModel->SetBboxDimention(XMFLOAT3(44.0f * transform->GetScale().x, 73.0f * transform->GetScale().y, 47.9f * transform->GetScale().z));
	particleModel->SetOrigin(XMFLOAT3(pos.x - (particleModel->GetBBoxDimention().x / 2), pos.y, pos.z - (particleModel->GetBBoxDimention().z / 2)));

	tower = new GameObject("Tower", appearance, transform, particleModel);
...
void Application::InitPlaneObjects()
{
	...
	XMFLOAT3 planePos = XMFLOAT3(-415.415f, 100.0f, 35.63f);
	...
	particleModel->SetBboxDimention(XMFLOAT3(10.0f, 3.8f, 16.2f));
	...
	Geometry AIplaneGeometry = OBJLoader::Load("Objects/Plane Objects/F-35_Lightning_II.obj", _pd3dDevice);

	XMFLOAT3 AIplanePos = XMFLOAT3(-415.415f, 100.0f, 35.63f);

	appearance = new Appearance(AIplaneGeometry, shinyMaterial);
	appearance->SetTextureRV(_pPlaneTex);

	transform = new Transform();
	transform->SetScale(1.0f, 1.0f, 1.0f);
	transform->SetPosition(AIplanePos);

	particleModel = new PlaneParticleModel(transform, 1.0f);
	particleModel->SetCollisionRadius(10.0f);

	GameObject* AIplaneBody = new GameObject("AIPlane", appearance, transform, particleModel);

	myPlane = new ControlledPlane(planeBody);
	aiPlane = new AIPlane(AIplaneBody);
}
...
void Application::CameraInput()
{
	...
	XMFLOAT3 planePos;
	XMFLOAT3 planeDirection;

	if (planeCam == 1)
	{
		planePos = aiPlane->GetPlaneBody()->GetTransform()->GetPosition();
		planeDirection = aiPlane->GetForwardVector();
	}

	if (planeCam == 2)
	{
		planePos = myPlane->GetPlaneBody()->GetTransform()->GetPosition();
		planeDirection = myPlane->GetForwardVector();
	}
...
void Application::PlaneUpdate(float t)
{
	...
	// Plane Body Updates
	myPlane->Update(t);
	aiPlane->Update(t, myPlane->GetPlanePosition());
...
void Application::Update(float t)
{
	...
	myPlane->GetPlaneBody()->GetParticleModel()->BBoxCollision(tower->GetParticleModel());
...
void Application::Draw()
{
	...
	material = tower->GetAppearance()->GetMaterial();
	...
	// Set world matrix
	cb.World = XMMatrixTranspose(tower->GetTransform()->GetWorldMatrix());

	// Set texture
	if (tower->GetAppearance()->HasTexture())
	{
		ID3D11ShaderResourceView* textureRV = tower->GetAppearance()->GetTextureRV();
		_pImmediateContext->PSSetShaderResources(0, 1, &textureRV);
		cb.HasTexture = 1.0f;
	}
	else
